...
class CGD_EER_EDD {
	...
	function verified_purchases( $purchases, $comment) {
		...
		if ( $comment->user_id != 0 ) {
			if ( edd_has_user_purchased($comment->user_id, $comment->comment_post_ID) ) {
				$purchases = 1;
			}
		}
	...
	function instructions() {
		...
		<code>
		cgd_eer_edd_print_reviews();
		</code>
		
		<h4>Review Form</h4>
		<p>Similar to above, show the review form.</p>
		<code>
		cgd_eer_edd_print_review_form();
		</code>
		<p></p>
		<?php
	}
...
function cgd_eer_edd_rating( $options = array() ) {
	return CGD_EER_EDD::rating($options);
}

function cgd_eer_edd_print_rating( $options = array() ) {
	echo cgd_eer_edd_rating($options);
}

function cgd_eer_edd_reviews( $options = array() ) {
	return CGD_EER_EDD::show_reviews($options);
}

function cgd_eer_edd_print_reviews( $options = array() ) {
	echo cgd_eer_edd_reviews($options);
}

function cgd_eer_edd_review_form( $options = array() ) {
	return CGD_EER_EDD::review_form($options);
}

function cgd_eer_edd_print_review_form( $options = array() ) {
	echo cgd_eer_edd_review_form($options);
}
