jQuery(function($) {
    ...
    $('#sortable-table tbody').sortable({
        ...
        var theOrder = $(this).sortable('toArray');
 		var nonce = $('#tdl_meta_box_nonce').attr('value');
        var data = {
            action: 'tdl_update_slide_order',
            postType: $(this).attr('data-post-type'),
            order: theOrder,
            tdl_meta_box_nonce: nonce
            };
    ...
    if($('td.post-title').parent().hasClass('type-home_slider') || $('td.post-title').parent().hasClass('type-tdl_slider')) {
