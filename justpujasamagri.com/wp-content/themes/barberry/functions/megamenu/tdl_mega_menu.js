...
jQuery(function($){
	var temp, type ;
	$(".tdl_megamenu_box input[type=checkbox]").live("click",function(){
		temp = $(this);
		if(temp.is(":checked"))
		{
	...
	$(".enable_textbox").live("click",function(){
	 ...
	 $("#menu-to-edit").find(".menu-item").each(function(){
		...
		{
			 $(this).find(".tdl_megamenu_box").hide();
			  $(this).find(".item-type-hmenu").hide();
			 if($(this).find(".tdl_megamenu_box input[type=checkbox]").is(":checked"))
			 {
				 $(this).find(".tdl_megamenu_box input[type=checkbox]").removeAttr("checked");
			 }
		}
		 else
		{
			...
			if(type="column")
			{
			 ...
			 $(this).find(".tdl_megamenu_box").show();
			  $(this).find(".tdl_megamenu_row_box").hide();
	...
	$(".tdl_megamenu_box input[type=checkbox]").each(function(){
	...
	type = $(this).find(".tdl_megamenu_box select").val();
	...
	var flag=false,dragel = ui;
	   ...
	   if( $(pindex).find(".tdl_megamenu_box input[type=checkbox]").is(":checked") )
	   flag = true;
	   else
	   flag = false;
	 ...
	 temp.find(".menu-item").each(function(){
		 ...
		 if(!$(this).hasClass("menu-item-depth-0"))
		{
			...
			$(this).find(".tdl_megamenu_row_box").show();
			$(this).find(".item-type-tdl-column").show();
			}
			else
			{
				 $(this).find(".tdl_megamenu_box").hide();
		 ...
		 else
		{
			 $(this).find(".tdl_megamenu_row_box").hide();
			 $(this).find(".tdl_megamenu_box").show();
			 $(this).find(".item-type-tdl-column").hide();
			 // $(this).find(".item-type-hmenu").show();
			 
			  if($(this).find(".tdl_megamenu_row_box input[type=checkbox]").is(":checked"))
			 {
				 $(this).find(".tdl_megamenu_row_box input[type=checkbox]").removeAttr("checked");
			 }
...
function set_column_tree(parent)
{
	...
	if(parent.find(".tdl_megamenu_box input[type=checkbox]").is(":checked"))
	{
	...
	$("#menu-to-edit").find(".menu-item").each(function(){
		...
		if(test>0)
		{
			...
			if(id==test)
			{
				obj.find(".item-type-tdl-column").show();
				obj.find(".tdl_megamenu_row_box").show();
			}
		}
...
function restruct_tree(parent)
{
	...
	$("#menu-to-edit").find(".menu-item").each(function(){
		...
		if(test>0)
		{
			...
			if(id==test)
			{
				obj.find(".item-type-tdl-column").hide();
				obj.find(".tdl_megamenu_row_box").hide();
			}
		}
